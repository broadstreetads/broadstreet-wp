jQuery(function ($) {
  ...
  $("#one-click-signup").click(function (e) {
    ...
    $.post(
      ajaxurl,
      {
        action: "register",
        email: email,
        nonce: broadstreet_ajax.register_nonce,
      },
      function (response) {
        if (response.success) {
          location.reload();
          ...
          alert(
            "There was an error creating a an account! Do you already have an account? If not, try again."
          );
        }
      },
      "json"
  ...
  $("#save-broadstreet").click(function () {
    ...
    jQuery.post(
      ajaxurl,
      {
        ...
        nonce: broadstreet_ajax.save_settings_nonce,
        api_key: $("#api_key").val(),
      ...
      function (response) {
        ...
        if (response.success) {
          ...
          if (response.key_valid) {
            ...
            $("#business_enabled").attr("disabled", false);
          } else {
            $("#network").append(
  ...
  $("#save_bs_advertiser").click(function (e) {
    ...
    $.post(
      ajaxurl,
      {
        ...
        nonce: broadstreet_ajax.create_advertiser_nonce,
        name: name,
      },
      function (response) {
        ...
        if (response.success) {
          $("#bs_advertiser_id").append(
            $(
              '<option value="' +
                response.advertiser.id +
                '" selected="selected">' +
                response.advertiser.name +
                "</option>"
            )
          );
        }
  ...
  function add_images(imgurl) {
    ...
    for (var i in imgurl) {
      ...
      uploadField.attr(
        "name",
        "bs_images[" + $("#bs_image_list").children().length + "]"
      );
      uploadField.val(imgurl[i]);

      $("#bs_image_list").append($("<li>").append(a).append(uploadField));
    }
  ...
  $("#bs-import").click(function (e) {
    ...
    $.post(
      ...
      {
        id: id,
        post_id: window.bs_post_id,
        action: "import_facebook",
        nonce: broadstreet_ajax.import_facebook_nonce,
      },
      function (response) {
        console.log(response);
        if (response.success) {
          ...
          if (
            confirm(
              "We were able to pull back a title and/or description for this business. Should we place it in the editor above?"
            )
          ) {
          ...
          if (!response.profile.charged) {
            alert(
              "Since we couldn't retrieve all of the standard information, this import is on the house."
            );
          }
        } else {
